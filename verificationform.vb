...
Public Class verificationform
            ...
            Using cn As New MySqlConnection(Module1.ConnectionString)
                ...
                Using tx As MySqlTransaction = cn.BeginTransaction()
                    Using comd As New MySqlCommand("INSERT INTO attendance (regno, name, date, day_name, session_name, course_code, course_name, session_id, timein, timeout) " &
                                                  ...
                                                  "ON DUPLICATE KEY UPDATE name = VALUES(name), day_name = VALUES(day_name), course_name = VALUES(course_name), session_id = VALUES(session_id)", cn, tx)
                ...
                End Using
            End Using
            AppLogger.Success("Clock-in: " & TextBox1.Text.Trim() & " | " & sessionName & " | " & courseCode)

            ListView1.Items.Clear()
            RefreshAttendanceList()

        Catch ex As Exception
            AppLogger.LogError("Clock-in error", ex)
    ...
    Private Sub verificationform_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            AppLogger.LogError("Load enrolled data (clock-in) failed", ex)
        End Try
        ...
        Try
            Using mysqlconn As New MySqlConnection(Module1.ConnectionString)
                ...
                Using cmdCount As New MySqlCommand("SELECT COUNT(*) FROM attendance WHERE date = @date AND (@session_id IS NULL OR session_id = @session_id)", mysqlconn)
                    cmdCount.Parameters.AddWithValue("@date", DateClockIn.Value.Date)
    ...
    Private Sub DateClockIn_ValueChanged(sender As Object, e As EventArgs) Handles DateClockIn.ValueChanged
        If Not txtDayName.ReadOnly Then
            txtDayName.Text = DateClockIn.Value.ToString("dddd")
        End If
    End Sub
    ...
    Public Sub ApplySessionInfo(ByVal info As SessionInfo)
        If info.SessionId <> currentSessionId Then
            ...
            currentPage = 0
        End If
        DateClockIn.Value = info.SessionDate
